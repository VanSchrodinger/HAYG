...
import pymysql,os
from .forms import CustForm,CustContactForm,CustSearchForm,JobInfoForm
...
def add_job(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            print('不合法的表单')
    return render('customer/custinfo.html',{'form': form})
...
def add_cust(request):  
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            # conn = pymysql.connect(host='127.0.0.1', user="root", password='password', database='jzyy', charset='utf8')
            # cursor = conn.cursor(cursor=pymysql.cursors.DictCursor)
            # try:
            #     sql = "insert into jzyy.customer_custinfo(cust_name, cust_fullname, invoice, profession, city, tel, website, location, email, finance, establish, source, develop_stage, salary_structure, products, size, competitor, worktime, welfare1, welfare2, creator, nature, is_share, work_category, introduction, culture, interview_process, cust_style) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            #     values = (cust_name, cust_fullname)
...
def custinfo(request): 
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            print('fuck2')
    sql ="select a.*,c.num from jzyy.customer_custinfo a left join (select cust_id,count(*) num from jzyy.customer_custcontact  group by cust_id) c on a.id = c.cust_id order by a.create_date desc"
    companies =CustInfo.objects.raw(sql)
